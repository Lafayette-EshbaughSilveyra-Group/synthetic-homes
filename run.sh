...
while true; do
  echo "Synthetic Homes Pipeline"
  echo "(c) 2026 Eshbaugh et al"
  echo "============================"
  echo "What would you like to run?"
  echo
  echo "1) Run full pipeline (scrape → GeoJSON → IDF → EnergyPlus)"
  echo "2) Run pipeline except for scraping (GeoJSON → IDF → EnergyPlus)"
  echo "3) Run occlusion experiments"
  echo "4) Exit"
  echo
