...
# Helper functions to adapt summary_stats.json entry for the calibrated labeler
...
def run(client, runs_per_sample=5):
    ...
    for eg_idx, eg in enumerate(experimental_groups):
        ...
        for element_idx, (text, eplus) in enumerate(eg):
            results_json = _summary_to_labeler_features(eplus)
            label_results = [
                label_data(
                    results_json, text, None, client,
